"""lore sync — read memories from GitLab issues and write wiki pages.

The sync command:
1. Connects to the GitLab API.
2. Finds the "LORE Memory Bank" issue (by title or the ``lore-memory-bank`` label).
3. Parses every comment for LORE Memory and LORE Pattern entries.
4. Creates / updates one wiki page per memory (``LORE-MEMORY-{id}``).
5. Rebuilds the ``LORE-INDEX`` wiki page (a markdown table mapping governed
...
# Required fields for a LORE Memory entry.
...
# Fields for a LORE Pattern Rule.
...
_MEMORY_HEADER_RE = re.compile(r"^LORE\s+Memory\s+#(\d+)", re.MULTILINE)
_PATTERN_HEADER_RE = re.compile(r"^LORE\s+Pattern\s+#(\d+)", re.MULTILINE)


def parse_memory(text: str) -> dict[str, Any] | None:
    """Extract a single LORE Memory from *text*.
...
def parse_pattern_rule(text: str) -> dict[str, Any] | None:
    """Extract a single LORE Pattern Rule from *text*.
...
def parse_all_entries(text: str) -> tuple[list[dict], list[dict]]:
    ...
    chunks = re.split(r"(?=^LORE\s+(?:Memory|Pattern)\s+#\d+)", text, flags=re.MULTILINE)
...
def _get_gitlab(args: argparse.Namespace):  # type: ignore[return]
    ...
    if args.project_id is None:
        print("ERROR: --project-id (or LORE_PROJECT_ID) is required for sync.", file=sys.stderr)
        sys.exit(1)
...
def _find_memory_bank_issue(project):  # type: ignore[return]
    """Locate the LORE Memory Bank issue by title or label."""
    # Try label first (fast).
    issues = project.issues.list(labels=["lore-memory-bank"], state="opened", per_page=1)
    if issues:
        return issues[0]

    # Fall back to title search.
    issues = project.issues.list(search="LORE Memory Bank", state="opened", per_page=5)
    for issue in issues:
        if "LORE Memory Bank" in issue.title:
            return issue
...
def _sync_wiki(project, memories: list[dict], patterns: list[dict]) -> tuple[int, int]:
    ...
    for mem in memories:
        slug = f"LORE-MEMORY-{mem['id']:03d}"
        content = _render_memory_page(mem)
        _upsert_wiki_page(project, slug, f"LORE Memory #{mem['id']:03d}", content)
        synced += 1

    for pat in patterns:
        slug = f"LORE-PATTERN-{pat['id']:03d}"
        content = _render_pattern_page(pat)
        _upsert_wiki_page(project, slug, f"LORE Pattern #{pat['id']:03d}", content)
        synced += 1
...
def _render_memory_page(mem: dict) -> str:
    """Render a LORE Memory dict as a Markdown wiki page."""
    lines = [f"# LORE Memory #{mem['id']:03d}", ""]
...
def _render_pattern_page(pat: dict) -> str:
    """Render a LORE Pattern dict as a Markdown wiki page."""
    lines = [f"# LORE Pattern #{pat['id']:03d}", ""]
...
def _rebuild_index(project, memories: list[dict]) -> int:
    """Rebuild the LORE-INDEX wiki page.  Returns number of index entries."""
    ...
    _upsert_wiki_page(project, "LORE-INDEX", "LORE Index", "\n".join(lines) + "\n")
    return len(file_map)
...
def run_sync(args: argparse.Namespace) -> int:
    """Execute the ``lore sync`` command."""
    _gl, project = _get_gitlab(args)

    issue = _find_memory_bank_issue(project)
    if issue is None:
        print("ERROR: Could not find the LORE Memory Bank issue.", file=sys.stderr)
        print("  Create an issue titled 'LORE Memory Bank' or add the label 'lore-memory-bank'.", file=sys.stderr)
        return 1
    ...
    if not memories and not patterns:
        print("No LORE memories or patterns found in issue comments.")
        return 0
