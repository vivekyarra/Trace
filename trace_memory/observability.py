...
from trace_memory.security import redact
...
class Metrics:
    ...
    def increment(self, name: str, *, labels: dict[str, str] | None = None, amount: int = 1) -> None:
        if not name.startswith("trace_") or not name.replace("_", "").isalnum():
            raise ValueError("metric names must be trace_ prefixed identifiers")
