"""Tracked SQL migrations and idempotent legacy LORE memory import."""
...
from lore.persistence import CockroachDatabase
...
_ID = re.compile(r"(?:LORE Memory|LORE-MEMORY-)\s*#?([0-9]+)", re.I)
...
class LegacyMemoryImporter:
    ...
    def import_text(self, content: str, *, strict: bool = True,
        ...
        blocks = re.split(r"(?=^LORE Memory\s+#?[0-9]+)", content, flags=re.M | re.I)
        parsed: list[Memory] = []
        errors: list[str] = []
        for block in blocks:
    ...
    def _parse(self, block: str) -> Memory:
        ...
        source = fields.get("source mr", "Legacy LORE import")
        ...
        return Memory(
            ...
            display_id=f"LORE-MEMORY-{int(match.group(1)):03d}",
            ...
            created_by_actor_type=ActorType.SYSTEM, created_by_actor_id="lore-importer",
        )
