...
from lore.ai.bedrock import BedrockEmbedder, BedrockReasoner
from lore.domain import ActorType, Memory, MemoryStatus, MemoryType
from lore.runtime.github import GitHubClient
...
class GitHubAutomation:
    ...
    def _pull_request(self, number: int, payload: dict[str, object]) -> None:
        ...
        if pull.get("merged_at"):
            envelope = self.reasoner.reason_json(
                system=("You are LOREKEEPER. Extract only durable architectural, security, incident, "
        ...
        envelope = self.reasoner.reason_json(
            system=("You are LORE GUARDKEEPER. Assess architectural conflicts, stated promises, code "
    ...
    def _issue(self, number: int, payload: dict[str, object]) -> None:
        ...
        envelope = self.reasoner.reason_json(
            system=("You are LORE SPECFORGE. Produce a failure pre-mortem, hard technical questions, "
    ...
    def _memory(self, pr_number: int, index: int, pr_title: str, item: AutomationDecision) -> Memory:
        ...
        display = f"LORE-MEMORY-{pr_number:03d}{index:02d}"
        return Memory(
            organization_id=self.organization_id, repository_id=self.repository_id,
            ...
            created_by_actor_type=ActorType.AGENT, created_by_actor_id="lorekeeper",
        )
