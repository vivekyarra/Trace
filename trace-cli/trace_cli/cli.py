"""Main CLI entry point for Trace.

Usage:
    trace sync   --project-id 123
    trace validate --from-file memories.txt
    trace stats  --from-file memories.txt
"""
...
from trace_cli import __version__
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="trace",
        description="Trace CLI — sync, validate, and report on architectural decision memories.",
    )
    parser.add_argument(
        "--version",
        action="version",
        version=f"trace-cli {__version__}",
    )
    parser.add_argument(
        "--project-id",
        type=int,
        default=None,
        help="GitLab project ID (env: TRACE_PROJECT_ID)",
    )
    parser.add_argument(
        "--gitlab-url",
        default=None,
    ...
    subparsers.add_parser(
        "sync",
        help="Read memories from the Trace Memory Bank issue and write wiki pages",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "sync":
        from trace_cli.sync import run_sync

        return run_sync(args)

    if args.command == "validate":
        from trace_cli.validate import run_validate

        return run_validate(args)

    if args.command == "stats":
        from trace_cli.stats import run_stats

        return run_stats(args)

    if args.command == "dashboard":
        from trace_cli.dashboard import generate_dashboard
        from trace_cli.sync import load_entries_from_file
        ...
        if args.from_file:
            ...
            if issue is None:
                print(
                    "ERROR: Could not find the Trace Memory Bank issue.",
                    file=sys.stderr,
                )
                return 1
