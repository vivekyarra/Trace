...
|---|---|---|
| Workflow definition | GitLab Duo flow with seven components: `triage_router`, `specforge_agent`, `guardkeeper_agent`, `reply_handler_agent`, `tracekeeper_agent`, `tracecast_agent`, and `onboarding_agent`. | `.gitlab/duo/flows/trace.yaml` |
| Catalog flow | A condensed GitLab AI Catalog flow with duplicated prompt material and a router. | `flows/flow.yml` |
| Runtime | Legacy FastAPI application with `/health`, `/webhook`, and `/tracecast`. Webhook routing performs agent work in-process. | `legacy/main.py` |
...
| Persistence | GitLab wiki pages are the authoritative memory store; `TRACE-INDEX` maps file paths to `TRACE-MEMORY-*` pages. | `legacy/core/memory.py` |
...
| CLI | `sync`, `validate`, `stats`, and `dashboard` operate on GitLab/Markdown records. | `trace-cli/trace_cli/cli.py` |
| Dashboard | Static HTML with representative hardcoded memories, not a live product/API view. | `public/index.html` |
| Tests | CLI-only unit tests for Markdown parsing, validation, statistics, and dashboard rendering. | `trace-cli/tests/` |
...
| Placeholder/no-op code | `.gitlab-ci.yml` has a placeholder Tracecast scheduled job; `agents/agent.yml.template` explicitly returns a placeholder response; `src/api/auth.py` is sample/insecure code and not an application runtime. |

## Preserved product value

- Trace’s institutional-memory voice and the core distinction between decisions, rejected alternatives, reasons, future implications, and dependencies.
- The workflow semantics: SPECFORGE, GUARDKEEPER, REPLY_HANDLER, Tracekeeper, Tracecast, ONBOARDING, and Trace Ask.
