"""
Tracekeeper agent: triggered on MR merge. Extracts architectural decisions from
...
logger = logging.getLogger("trace.tracekeeper")


async def run_tracekeeper(payload: dict) -> None:
    """
    Run the Tracekeeper agent on a merge request webhook payload. Extracts decisions
    ...
    try:
        ...
        if not changed_files:
            logger.warning("Tracekeeper: MR !%s has no changed files, skipping", mr_iid)
            return
        ...
        meaningful = [
            ...
            and not (n.get("body") or "").strip().startswith("🧠 Trace")
        ]
        ...
        # — Step 6: Call Claude —
        system_prompt = claude.load_prompt("tracekeeper")
        ...
        try:
            ...
            logger.error("Tracekeeper: Failed to parse Claude JSON: %s", e, exc_info=True)
            gitlab.post_mr_comment(
                mr_iid,
        ...
        logger.error("Tracekeeper failed: %s", e, exc_info=True)
        try:
